...
public class Busqueda extends JFrame {
	...
	private void loadReservationsToTable() {
		...
		for (Reservation reservation : reservations) {
			modelo.addRow(new Object[] { reservation.getId(), reservation.getFromDate(), reservation.getToDate(),
					reservation.getPrice(), reservation.getPaymentMethod() });
		}
	}
		...
		Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
				.ifPresent(row -> {
					...
					try {
						...
						fromDate = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 1).toString(),
								dateFormat);
						toDate = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 2).toString(),
								dateFormat);
					} catch (DateTimeException e) {
						throw new RuntimeException(e);
					}
					this.reservasView.clearValues();

					String price = calculatePrice(fromDate, toDate);
	...
	private String calculatePrice(LocalDate fromDate, LocalDate toDate) {
